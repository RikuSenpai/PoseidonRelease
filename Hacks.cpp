...
void SpecList()
{
	...
	for (int i = 0; i < Interfaces::EntList->GetHighestEntityIndex(); i++)
	{
		...
		if (pEntityity &&  pEntityity != pLocal)
		{
			if (Interfaces::Engine->GetPlayerInfo(i, &pinfo) && !pEntityity->IsAlive() && !pEntityity->IsDormant())
			{
				HANDLE obs = pEntityity->GetObserverTargetHandle();
...
void Hacks::MoveHacks(CUserCmd *pCmd,bool &bSendPacket)
{
	...
	if (AirStuckKey > 0 && GUI.GetKeyState(AirStuckKey))
	{
		...
		{
			pCmd->tick_count = INT_FAST16_MAX;
		}
