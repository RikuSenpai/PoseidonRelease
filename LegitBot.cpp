...
void CLegitBot::DoAimbot(CUserCmd *pCmd)
{
	...
	if (Menu::Window.LegitBotTab.LegitAA.GetState())
	{
		{
			jitter2 = !jitter2;
			pCmd->viewangles.y = jitter2 ? c_beam->real + 0 : c_beam->real - 58;
		}
	}
...
bool CLegitBot::TargetMeetsTriggerRequirements(IClientEntity* pEntityity)
{
	if (pEntityity && pEntityity->IsDormant() == false && pEntityity->IsAlive() && pEntityity->GetIndex() != hackManager.pLocal()->GetIndex() && pEntityity->GetIndex() < 65)
	{
		if (pEntityity->GetTeamNum() != hackManager.pLocal()->GetTeamNum() || Menu::Window.LegitBotTab.AimbotFriendlyFire.GetState())
		{
...
bool CLegitBot::TargetMeetsRequirements(IClientEntity* pEntityity)
{
		...
		if (pClientClass->m_ClassID == (int)CSGOClassID::CCSPlayer && Interfaces::Engine->GetPlayerInfo(pEntityity->GetIndex(), &pinfo))
		{
			if (pEntityity->GetTeamNum() != hackManager.pLocal()->GetTeamNum() || Menu::Window.LegitBotTab.AimbotFriendlyFire.GetState())
			{
				...
				if (Menu::Window.LegitBotTab.AimbotSmokeCheck.GetState()) {
					...
					if (GoesThroughSmokeFunction(hackManager.pLocal()->GetEyePosition(), pEntityity->GetBonePos(8)))
						return false;
				}
				...
				if (Multihitbox)
				{
					if (!pEntityity->HasGunGameImmunity())
					{
						if (GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 0) || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 8) || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 4 || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 1 || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 2 || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 3 || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 5 || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 7 || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 9 || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 10 || GameUtils::IsVisible(hackManager.pLocal(), pEntityity, 6))))))))))
							return true;
						else
							return false;
					}
				...
				{
					if (!pEntityity->HasGunGameImmunity() && GameUtils::IsVisible(hackManager.pLocal(), pEntityity, HitBox))
					{
						return true;
					}
				}
...
float CLegitBot::FovToPlayer(Vector ViewOffSet, Vector View, IClientEntity* pEntityity, int aHitBox)
{
	...
	for (int i = 0; i < Interfaces::EntList->GetHighestEntityIndex(); i++)
	{
		...
		if (TargetMeetsRequirements(pEntityity))
		{
			if (Multihitbox)
			{
				...
				float fov1 = FovToPlayer(ViewOffset, View, pEntityity, 0);
				float fov2 = FovToPlayer(ViewOffset, View, pEntityity, 4);
				float fov3 = FovToPlayer(ViewOffset, View, pEntityity, 6);
