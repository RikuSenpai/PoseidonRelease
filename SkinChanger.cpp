...
#define iKnife Menu::Window.SettingsTab.t_knife_index.GetIndex() 

int SkinChanger::get_skin_id_knife()
{
	int id = Menu::Window.SettingsTab.t_knife_skin_id.GetIndex();
...
{
	int id = Menu::Window.SettingsTab.t_sniperAWP_skin_id.GetIndex();
...
{
	int id = Menu::Window.SettingsTab.t_sniperSCAR_skin_id.GetIndex();
...
#define flKnifeWear Menu::Window.SettingsTab.t_knife_wear.GetValue() / 1000 //slider for the wear
...
{
	CBaseViewModel* pViewModel = (CBaseViewModel*)Interfaces::EntList->GetClientEntityFromHandle(*(HANDLE*)((DWORD)pLocal + 0x32F8));
	if (!pViewModel)
		return false;
	DWORD hViewModelWeapon = pViewModel->GetWeapon();
	CBaseAttributableItem* pViewModelWeapon = (CBaseAttributableItem*)Interfaces::EntList->GetClientEntityFromHandle((HANDLE)hViewModelWeapon);
